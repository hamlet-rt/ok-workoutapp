...
fun ICorChainDsl<WrkContext>.stubSearchSuccess(title: String) = worker {
    this.title = title
    on { stubCase == WrkStubs.SUCCESS && state == WrkState.RUNNING }
    handle {
        state = WrkState.FINISHING
        tngResponse = WrkTngStub.prepareResult {
            tngRequest.id.takeIf { it != WrkTngId.NONE }?.also { this.id = it }
        }
        tngsResponse.addAll(WrkTngStub.prepareSearchList(tngResponse.title, WrkTngType.POWER))
    }
}